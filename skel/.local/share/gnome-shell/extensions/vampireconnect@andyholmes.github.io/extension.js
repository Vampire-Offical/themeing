...
const ServiceIndicator = GObject.registerClass({
    GTypeName: 'VampireConnectServiceIndicator',
    ...
    _init() {
        ...
        this.settings = new Gio.Settings({
            settings_schema: Config.GSCHEMA.lookup(
                'org.gnome.Shell.Extensions.VampireConnect',
                null
            ),
            path: '/org/gnome/shell/extensions/vampireconnect/',
        });
        ...
        this._indicator.gicon = Extension.getIcon(
            'org.gnome.Shell.Extensions.VampireConnect-symbolic'
        );
        this._indicator.visible = false;

        AggregateMenu._indicators.insert_child_at_index(this, 0);
        AggregateMenu._vampireconnect = this;
        ...
        this.deviceSection.actor.add_style_class_name('vampireconnect-device-section');
        this.settings.bind(
            'show-indicators',
    ...
    _preferences() {
        Gio.Subprocess.new([`${Extension.path}/vampireconnect-preferences`], 0);
    }
    ...
    _onDeviceAdded(service, device) {
        try {
            ...
            device.settings = new Gio.Settings({
                settings_schema: Config.GSCHEMA.lookup(
                    'org.gnome.Shell.Extensions.VampireConnect.Device',
                    true
                ),
                path: `/org/gnome/shell/extensions/vampireconnect/device/${device.id}/`,
            });
    ...
    destroy() {
        ...
        delete AggregateMenu._vampireconnect;
        super.destroy();
    }
});
...
function init() {
    ...
    // and they're only used *by* VampireConnect, they should be okay to leave.
    Utils.installService();

    // These modify the notification source for VampireConnect's GNotifications and
    // need to be active even when the extension is disabled (eg. lock screen).
    // Since they *only* affect notifications from VampireConnect, it should be okay
    // to leave them applied.
    Notification.patchVampireConnectNotificationSource();

"""
nautilus-vampireconnect.py - A Nautilus extension for sending files via VampireConnect.
...
SERVICE_NAME = 'org.gnome.Shell.Extensions.VampireConnect'
SERVICE_PATH = '/org/gnome/Shell/Extensions/VampireConnect'

# Init gettext translations
LOCALE_DIR = os.path.join(GLib.get_user_data_dir(),
                          'gnome-shell', 'extensions',
                          'vampireconnect@andyholmes.github.io', 'locale')
...
try:
    ...
    print('VampireConnect: {0}'.format(e.strerror))
    i18n = gettext.translation(SERVICE_NAME,
                               localedir=LOCALE_DIR,
                               fallback=True)
    _ = i18n.gettext
...
class VampireConnectShareExtension(GObject.Object, FileManager.MenuProvider):
    """A context menu for sending files via VampireConnect."""
    ...
    def _on_g_signal(self, proxy, sender_name, signal_name, parameters):
        ...
        if signal_name == 'InterfacesAdded':
            for object_path, props in objects.items():
                props = props['org.gnome.Shell.Extensions.VampireConnect.Device']
    ...
    def get_file_items(self, window, files):
        ...
        menu = FileManager.MenuItem(
            name='VampireConnectShareExtension::Devices',
        ...
        for name, action_group in devices:
            item = FileManager.MenuItem(
                name='VampireConnectShareExtension::Device' + name,
                label=name
            )

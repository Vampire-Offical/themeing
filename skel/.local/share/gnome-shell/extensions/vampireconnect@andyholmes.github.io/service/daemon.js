...
// See https://github.com/VampireConnect/gnome-shell-extension-vampireconnect/issues/930
...
/**
 * Class representing the VampireConnect service daemon.
 */
const Service = GObject.registerClass({
    GTypeName: 'VampireConnectService',
}, class Service extends Gtk.Application {

    _init() {
        super._init({
            application_id: 'org.gnome.Shell.Extensions.VampireConnect',
            flags: Gio.ApplicationFlags.HANDLES_OPEN,
            resource_base_path: '/org/gnome/Shell/Extensions/VampireConnect',
        });

        GLib.set_prgname('vampireconnect');
        GLib.set_application_name('VampireConnect');
    ...
    _preferences() {
        Gio.Subprocess.new(
            [`${Config.PACKAGE_DATADIR}/vampireconnect-preferences`],
    ...
    notify_error(error) {
        try {
            ...
            notif.set_title(`VampireConnect: ${error.name.trim()}`);
            notif.set_body(body);
            notif.set_icon(icon);
    ...
    vfunc_open(files, hint) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                logError(e, `VampireConnect: Opening ${file.get_uri()}`);
            }
        }
    }
    ...
    _cliAction(id, name, parameter = null) {
        ...
        Gio.DBus.session.call_sync(
            ...
            `/org/gnome/Shell/Extensions/VampireConnect/Device/${id}`,
            'org.gtk.Actions',
            'Activate',
    ...
    _cliListDevices(full = true) {
        ...
        for (let object of Object.values(variant)) {
            ...
            device = object['org.gnome.Shell.Extensions.VampireConnect.Device'];
    ...
    _cliNotify(id, options) {
        ...
        let icon = null;
        let nid = `${Date.now()}`;
        let appName = 'VampireConnect CLI';
        ...
        if (options.contains('notification-icon')) {
            ...
            icon = new Gio.ThemedIcon({
                name: 'org.gnome.Shell.Extensions.VampireConnect',
            });
        }
    ...
    vfunc_handle_local_options(options) {
        try {
            if (options.contains('version')) {
                print(`VampireConnect ${Config.PACKAGE_VERSION}`);
                return 0;
            }

...
var Player = GObject.registerClass({
    GTypeName: 'VampireConnectMediaPlayerInterface',
    Properties: {
    ...
    get DesktopEntry() {
        if (this._DesktopEntry === undefined)
            return 'org.gnome.Shell.Extensions.VampireConnect';
...
const PlayerProxy = GObject.registerClass({
    GTypeName: 'VampireConnectMPRISPlayer',
...
var Manager = GObject.registerClass({
    GTypeName: 'VampireConnectMPRISManager',
    Signals: {
        'player-added': {
    ...
    async _loadPlayers() {
        try {
            ...
            for (let i = 0, len = names.length; i < len; i++) {
                ...
                if (!name.includes('VampireConnect'))
     ...
     *
     * @param {string} identity - A player name
     * @return {VampireConnectMPRISPlayer|null} A player or %null
     */
    getPlayer(identity) {
        for (const player of this._players.values()) {
            if (player.Identity === identity)

...
const _ = Extension._;
const APP_ID = 'org.gnome.Shell.Extensions.VampireConnect';
const APP_PATH = '/org/gnome/Shell/Extensions/VampireConnect';
...
const NotificationBanner = GObject.registerClass({
    GTypeName: 'VampireConnectNotificationBanner',
...
const Source = GObject.registerClass({
    GTypeName: 'VampireConnectNotificationSource',
}, class Source extends NotificationDaemon.GtkNotificationDaemonAppSource {

    _closeVampireConnectNotification(notification, reason) {
    ...
    addNotification(notificationId, notificationParams, showBanner) {
        ...
        // Device Notification
            ...
            notification.connect('destroy', (notification, reason) => {
                this._closeVampireConnectNotification(notification, reason);
...
/**
 * If there is an active GtkNotificationDaemonAppSource for VampireConnect when the
 * extension is loaded, it has to be patched in place.
 */
function patchVampireConnectNotificationSource() {
    const source = Main.notificationDaemon._gtkNotificationDaemon._sources[APP_ID];

    if (source !== undefined) {
        // Patch in the subclassed methods
        source._closeVampireConnectNotification = Source.prototype._closeVampireConnectNotification;
        ...
        for (const notification of Object.values(source._notifications)) {
            ...
            const _id = notification.connect('destroy', (notification, reason) => {
                source._closeVampireConnectNotification(notification, reason);
...
/**
 * Wrap GtkNotificationDaemon._ensureAppSource() to patch VampireConnect's app source
...
function patchGtkNotificationSources() {
    ...
    const addNotification = function (notificationId, notificationParams, showBanner) {
        ...
        notification.connect('destroy', (notification, reason) => {
            this._withdrawVampireConnectNotification(notification, reason);
    ...
    // eslint-disable-next-line func-style
    const _withdrawVampireConnectNotification = function (id, notification, reason) {
        if (reason !== MessageTray.NotificationDestroyedReason.DISMISSED)
            return;
    ...
    NotificationDaemon.GtkNotificationDaemonAppSource.prototype._withdrawVampireConnectNotification = _withdrawVampireConnectNotification;
}
...
function unpatchGtkNotificationSources() {
    ...
    delete NotificationDaemon.GtkNotificationDaemonAppSource.prototype._withdrawVampireConnectNotification;
}

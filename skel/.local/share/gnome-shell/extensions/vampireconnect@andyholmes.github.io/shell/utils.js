...
/**
 * Get a themed icon, using fallbacks from VampireConnect's GResource when necessary.
 *
...
function getIcon(name) {
    if (getIcon._resource === undefined) {
        ...
        const iconPath = 'resource://org/gnome/Shell/Extensions/VampireConnect/icons';
        const iconNames = [
 ...
 * necessary.
 *
 * @param {string} relativePath - A path relative to VampireConnect's resource path
...
/**
 * Install the files necessary for the VampireConnect service to run.
 */
function installService() {
    ...
    const fileManagers = [
        [`${dataDir}/nautilus-python/extensions`, 'nautilus-vampireconnect.py'],
        [`${dataDir}/nemo-python/extensions`, 'nemo-vampireconnect.py'],
    ];

    // WebExtension Manifests
    const manifestFile = 'org.gnome.shell.extensions.vampireconnect.json';
    ...
    if (Config.IS_USER) {
        ...
        if (!_installResource(dbusDir, dbusFile, `${dbusFile}.in`))
            throw Error('VampireConnect: Failed to install DBus Service');
        ...
        // File Manager Extensions
        const target = `${Config.PACKAGE_DATADIR}/nautilus-vampireconnect.py`;
